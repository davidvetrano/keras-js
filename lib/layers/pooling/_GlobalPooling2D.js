 ...
 */
class _GlobalPooling2D extends _Layer2.default {
  /**
  ...
  _callCPU(x) {
    ...
    for (let i = 0, len = channels; i < len; i++) {
      if (this.poolingFunc === 'max') {
        this.output.tensor.set(i, _ndarrayOps2.default.sup(x.tensor.pick(null, null, i)));
      } else if (this.poolingFunc === 'average') {
        this.output.tensor.set(i, _ndarrayOps2.default.sum(x.tensor.pick(null, null, i)) / (rows * cols));
      }
    }
  }
  ...
  _callGPU(x) {
    ...
    if (!this.output) {
      this.output = new _Tensor2.default([], [this.inputShape[2]]);
