 ...
 */
class UpSampling1D extends _Layer2.default {
  /**
  ...
  _callCPU(x) {
    ...
    for (let i = 0; i < this.size; i++) {
      _ndarrayOps2.default.assign(this.output.tensor.lo(i, 0).step(this.size, 1), x.tensor);
    }
  }
  ...
  _createIndexMap() {
    ...
    for (let i = 0; i < this.size; i++) {
      _ndarrayOps2.default.assign(this.rowIndexMap.tensor.lo(i, 0).step(this.size, 1), indicesRow.tensor);
      _ndarrayOps2.default.assign(this.colIndexMap.tensor.lo(i, 0).step(this.size, 1), indicesCol.tensor);
    }
