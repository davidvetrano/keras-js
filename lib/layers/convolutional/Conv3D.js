 ...
 * Conv3D layer class
 */
class Conv3D extends _Layer2.default {
  /**
  ...
  _vol2col(x) {
    ...
    if (!this.volColsMat) {
      this.volColsMat = new _Tensor2.default([], [nbPatches, patchLen]);
    }
    ...
    let patch = new _Tensor2.default([], [kernelDim1, kernelDim2, kernelDim3, inputChannels]);
    let offset = 0;
  ...
  _callCPU(x) {
    ...
    (0, _ndarrayGemm2.default)(matMul.tensor, this.volColsMat.tensor, this.wRowsMat.tensor, 1, 1);

    this.output = new _Tensor2.default([], this.outputShape);

    let outputChannelRaveled = new _Tensor2.default([], [outputDim1 * outputDim2 * outputDim3]);
    let outputChannel = new _Tensor2.default([], [outputDim1, outputDim2, outputDim3]);
    for (let n = 0; n < nbFilter; n++) {
      ...
      _ndarrayOps2.default.assign(this.output.tensor.pick(null, null, null, n), outputChannel.tensor);
    }
  ...
  _createIndexMap(indicesForReshaped = {}) {
    ...
    let indicesRowPatch = new _Tensor2.default([], [kernelDim1, kernelDim2, kernelDim3, inputChannels]);
    let indicesColPatch = new _Tensor2.default([], [kernelDim1, kernelDim2, kernelDim3, inputChannels]);
    let offset = 0;
    for (let i = 0, limit = inputDim1 - kernelDim1Dilated; i <= limit; i += this.strides[0]) {
      for (let j = 0, limit = inputDim2 - kernelDim2Dilated; j <= limit; j += this.strides[1]) {
        for (let k = 0, limit = inputDim3 - kernelDim3Dilated; k <= limit; k += this.strides[2]) {
          _ndarrayOps2.default.assign(indicesRowPatch.tensor, indicesRow.tensor.hi(i + kernelDim1Dilated, j + kernelDim2Dilated, k + kernelDim3Dilated, inputChannels).lo(i, j, k, 0).step(this.dilationRate[0], this.dilationRate[1], this.dilationRate[2], 1));
          _ndarrayOps2.default.assign(indicesColPatch.tensor, indicesCol.tensor.hi(i + kernelDim1Dilated, j + kernelDim2Dilated, k + kernelDim3Dilated, inputChannels).lo(i, j, k, 0).step(this.dilationRate[0], this.dilationRate[1], this.dilationRate[2], 1));
